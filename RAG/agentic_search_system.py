...
def process_retry_logic(count, collection_processed_name, new_ref_collection, 
    ...
    """
    change_prompt_state_or_add(uri,db.name,'prompt',old_prompt,'N')

    # Loop as long as there are retries left or missing references exist
    while count > 0 and not missing_ref_df.empty:
        #call evaluator to choose from list of prompts deemed effective in history else generate a new prompt
        new_prompt=evaluator(old_prompt,'prompt')
        #generated prompt is now the 'old' prompt to be added to db/ change effectiveness
        ...
        #able to output a df of new data found, is uty
        ...
        missing_ref_df_new = retrieve_missing_references(valid, retry, statement_df)
        effectiveness_state(missing_ref_df_new=missing_ref_df_new,missing_ref_df_initial=missing_ref_df,prompt=old_prompt,collection='prompt')
        missing_ref_df=missing_ref_df_new
...
def agentic_search(collection_processed_name,new_ref_collection, 
    ...
    if not missing_ref_df.empty:
        ...
        print('All statements in initial search had satisfactory keywords generated and hence satisfactory search results. Agentic search loop shall not start.')